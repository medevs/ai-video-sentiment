...
export function Inference({ quota }: InferenceProps) {
  ...
  /**
   * Calculates the average scores for emotions and sentiments.
   * @returns An object containing the top emotion and sentiment.
   */
  const getAverageScores = () => {
    ...
    analysis.analysis.utterances.forEach((utterance) => {
      utterance.emotions.forEach((emotion) => {
        // Safe initialization using nullish coalescing assignment
        emotionScores[emotion.label] ??= [];
        // TypeScript needs a non-null assertion or type guard here
        const scores = emotionScores[emotion.label];
        if (scores) {
          scores.push(emotion.confidence);
        }
      });
      utterance.sentiments.forEach((sentiment) => {
        ...
        // TypeScript needs a non-null assertion or type guard here
        const scores = sentimentScores[sentiment.label];
        if (scores) {
          scores.push(sentiment.confidence);
        }
      });
    });

    /**
     * Calculate the average confidence for each emotion and sentiment.
     */
    ...
    /**
     * Calculate the average confidence for each sentiment.
     */
    ...
    <div className="flex h-fit w-full flex-col gap-3 md:w-1/2">
        ...
        <div className="flex h-fit w-full flex-wrap items-center justify-center gap-4 rounded-xl border border-gray-200 p-4 sm:gap-8 sm:px-6">
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm text-gray-500">
              ...
              {averages.topEmotion && (averages.topEmotion.confidence * 100).toFixed(0)}%)
            </span>
          </div>
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm text-gray-500">
              ...
              {averages.topSentiment && (averages.topSentiment.confidence * 100).toFixed(0)}%)
            </span>
          </div>
        </div>
      ) : (
      ...
      {analysis ? (
        <div className="flex flex-col gap-2">
          {analysis?.analysis.utterances.map((utterance, _i) => {
            return (
              <div
                key={
